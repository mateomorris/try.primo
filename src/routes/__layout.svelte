<script>
	...
	onMount(async () => {
		const res = await fetch('https://api.primo.so/try-primo.json').then((res) => res.json());
		if (!data) {
			data = res;
		}
	});
	...
	async function saveData(updatedSite) {
		localStorage.setItem('saved_site', JSON.stringify(updatedSite));
		$saved = true;
	}

	let data = (browser && JSON.parse(localStorage.getItem('saved_site'))) || null;
</script>

{#if browser}
	<Primo {data} on:save={async ({ detail: data }) => saveData(data)} />
{/if}
<div id="app-version">
	<span>Try Primo v{tryprimo.version}</span>
